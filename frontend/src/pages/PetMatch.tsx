...
export function PetMatch() {
    ...
    const lostPets = useMemo(() => 
        pulses.filter(p => p.content.toLowerCase().includes('lost') || p.content.toLowerCase().includes('missing')), 
    [pulses]);
    
    const foundPets = useMemo(() => 
        pulses.filter(p => p.content.toLowerCase().includes('found') || p.content.toLowerCase().includes('seen')), 
    [pulses]);
    ...
    return (
        <AppLayout title="Pet Guardian">
            <div style="padding:16px;display:flex;flex-direction:column;gap:20px;max-width:800px;margin:0 auto;">
                {/* AI Header */}
                <div class="card animate-slide-up" style="padding:20px;background:linear-gradient(135deg, var(--accent-subtle) 0%, var(--surface) 100%);border:1px solid var(--accent);display:flex;align-items:center;justify-content:space-between;gap:16px;">
                    <div style="flex:1;">
                        ...
                        <div style="display:flex;flex-direction:column;gap:12px;">
                            {Object.entries(matches).map(([lostId, matchResults]) => {
                                ...
                                return matchResults.map((m) => {
                                    ...
                                    return (
                                        <div key={`${lostId}-${m.id}`} class="card animate-slide-up" style="padding:0;overflow:hidden;border-left:4px solid var(--warning);">
                                            <div style="padding:12px 16px;background:var(--bg-muted);display:flex;justify-content:space-between;align-items:center;">
                                                ...
                                                <span style="font-size:11px;color:var(--text-tertiary);">{foundPulse.userName}'s report</span>
                                            </div>
                                            <div style="padding:16px;display:grid;grid-template-columns:1fr auto 1fr;gap:20px;align-items:center;">
                                                <div style="font-size:13px;color:var(--text);">
                                                    <span style="font-size:10px;font-weight:800;color:var(--danger);display:block;margin-bottom:4px;">LOST</span>
                                                    {lostPulse.content}
                                                </div>
                                                <ArrowRight size={18} style="color:var(--text-tertiary);" />
                                                <div style="font-size:13px;color:var(--text);">
                                                    <span style="font-size:10px;font-weight:800;color:var(--success);display:block;margin-bottom:4px;">FOUND</span>
                                                    {foundPulse.content}
                                                </div>
                                            </div>
                ...
                <section>
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        {loading && <div style="color:var(--text-secondary);font-size:13px;padding:20px;text-align:center;">Scanning neighborhood...</div>}
                        {!loading && pulses.length === 0 && <div style="color:var(--text-secondary);font-size:13px;padding:20px;text-align:center;">No pet reports found in your area.</div>}
                        {pulses.map((pet, i) => {
                            const isLost = pet.content.toLowerCase().includes('lost') || pet.content.toLowerCase().includes('missing');
                            return (
                                <div key={pet.id} class="card animate-slide-up" style={`padding:14px;display:flex;gap:14px;animation-delay:${i * 40}ms;`}>
                                    <div style={`width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;background:${isLost ? 'var(--danger-subtle)' : 'var(--success-subtle)'};color:${isLost ? 'var(--danger)' : 'var(--success)'};`}>
                                        <PawPrint size={20} />
                                    </div>
                                    <div style="flex:1;">
                                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                                            <span style={`font-size:10px;font-weight:800;padding:2px 6px;border-radius:4px;background:${isLost ? 'var(--danger)' : 'var(--success)'};color:white;`}>
                                                {isLost ? 'LOST' : 'FOUND'}
                                            </span>
                                            <span style="font-size:13px;font-weight:700;color:var(--text);">{pet.userName}</span>
