...
import { Bell, X, ShieldAlert } from 'lucide-preact';
		...
		if (permissionStatus === 'default' && typeof Notification !== 'undefined') {
			return (
				<div style="position:fixed;bottom:24px;right:24px;z-index:90;display:flex;align-items:center;gap:12px;padding:12px 16px;background:var(--surface);border:1px solid var(--border);border-radius:12px;box-shadow:var(--shadow-lg);border-left:4px solid var(--accent);animate-fade-in">
					<div style="flex:1">
						<p style="margin:0;font-size:12px;font-weight:600;color:var(--text);">Enable Hero Notifications?</p>
						<p style="margin:2px 0 0;font-size:11px;color:var(--text-secondary);">Get alerted instantly when your skills are needed.</p>
					</div>
					<button 
			...
			<div style="padding:16px;background:linear-gradient(135deg, var(--type-emergency-bg), rgba(255,255,255,0.05));">
				...
				<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:12px;padding:12px;margin-bottom:12px;">
					<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
						<img src={activeAlert.userAvatar} style="width:28px;height:28px;border-radius:50%;border:2px solid var(--accent-muted);" />
						<span style="font-size:13px;font-weight:700;color:var(--text);">{activeAlert.userName}</span>
				...
				<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
					{activeAlert.requiredSkills?.map(skill => (
						<span key={skill} style="background:var(--accent);color:white;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;">
							{skill}
						</span>
					))}
				</div>
